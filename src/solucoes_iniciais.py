...
class GeradorSolucoes:
    ...
    def gerar_solucao_inicial(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Randomiza a ordem de alocação das equipes
        indices_equipes = list(range(equipes_alocadas))
        np.random.shuffle(indices_equipes)
        
        for k in indices_equipes:
            # Adiciona randomização na escolha da base
            if np.random.random() < 0.7:  # 70% chance de escolher base central
                base_escolhida = bases_ordenadas[k % len(bases_ordenadas)]
            else:  # 30% chance de escolher base aleatória
                base_escolhida = np.random.choice(self.m_bases)
            y_jk[base_escolhida, k] = 1
        
        # Passo 2: Atribui ativos às bases (com randomização)
        for i in range(self.n_ativos):
            ...
            if len(bases_com_equipes) > 0:
                # Adiciona randomização na escolha da base
                if np.random.random() < 0.8:  # 80% chance de escolher base mais próxima
                    distancias_validas = self.distancias[i, bases_com_equipes]
                    base_mais_proxima = bases_com_equipes[np.argmin(distancias_validas)]
                else:  # 20% chance de escolher base aleatória com equipe
                    base_mais_proxima = np.random.choice(bases_com_equipes)
